...
Adapting {\em data mining algorithms} for the analysis of data from {\em dynamical systems}
...
{\footnotesize
In both cases, the identified eigenvectors are one-to-one with the arglength along the spiral and the height of the Swiss roll. 
\par}
...
%\node [right=0.1in of b,text width=2in] {\footnotesize Embeddings recover $p$ and $t$ \newline Relative importance changes \par};
...
As we increase $\lambda$, the eigenvectors which correspond to $p$ and $t$ move farther down in the spectrum
...
\node [left=-0.1in of a, text width=0.75in] {\scriptsize Diffusion maps eigenvalue spectra \par};
\node [left=-0.1in of d, text width=0.75in] {\scriptsize Diffusion maps embeddings \par};
...
\begin{itemize}
\item Look at the ratio of the two eigenvalues corresponding to unique eigendirections
\item If this ratio is small, data are effectively one-dimensional
\item If this ratio is large, data are effectively two-dimensional
\end{itemize}

\centering
\includegraphics[width=2.5in]{tmax_lambda_transition}
...
\newfootnote{Dsilva {\em et al}, {\em Current Biology}, 2015.}
...
\begin{itemize}
\item Can automatically register and order complex imaging data sets
\item Can extract representative developmental trajectories to uncover stereotypic dynamics
\end{itemize}

\vfill

\includegraphics[height=1.25in]{synchronization_schematic}
\hfill
\includegraphics[height=1.25in]{drosophila_data_ordered}
