...
\title{Data-Driven Methods for the Analysis \\of Multiscale Stochastic Systems}
\author{{\bf Carmeline Dsilva}\inst{1}, Ronen Talmon\inst{2}, \\Ronald Coifman\inst{2}, and Yannis Kevrekidis\inst{1,3} }
\institute{\inst{1} Department of Chemical and Biological Engineering, Princeton University  \\ %
                      \inst{2} Department of Mathematics, Yale University \\
                      \inst{3} Program in Applied and Computational Mathematics, Princeton University }
...
\begin{frame}{Data Reduction Techniques}

\centering
Common technique: Principal Component Analysis \\
{\em \scriptsize Project onto hyperplane which captures maximum variance \par }

\begin{tikzpicture}

\node(a){\includegraphics[width=2cm]{PCA_3D}};
\node(texta)[below=0cm of a, text width=4.5cm, align=center] {{\scriptsize High dimensional data on low-dimensional structure \par}};
\node[right=2cm of a](b){\includegraphics[width=2cm]{PCA_2D}};
\node(texba)[below=0cm of b, text width=4cm, align=center] {{\scriptsize Reduced dimensionality \par}};
\draw[->] (a) -- (b) node[above, midway]{{\small Data reduction}};
\end{tikzpicture}

For reduction of nonlinear data sets: {\em Diffusion Maps}

\vspace{-0.2cm}
\begin{tikzpicture}
        \node (fig1) {\includegraphics[width=2cm]{dmaps_spiral_black_plane_large.jpg}};
        %\draw[red,->] (-0.15,-1.2) -- (0.9, -0.9);
        %\draw[red,->] (-0.15,-1.2) -- (-1, 0.4);
        \node[right=2.5cm of fig1] (fig2) {\includegraphics[width=2cm]{dmaps_spiral_color_plane_large.jpg}};
        \draw [->] (fig1.east) -- (fig2.west);
        
        \node[below=0cm of fig1, text width=5cm, align=center]{{\scriptsize Data on a spiral \\  {\em PCA will find two-dimensional plane} \par}};
        
        \node[below=0cm of fig2, text width=0.5\textwidth, align=center]{{\scriptsize Data colored by first diffusion maps coordinate \par}};
        
    \end{tikzpicture}

	\newfootnote{Shlens, ``A Tutorial on Principal Component Analysis; Coifman {\em et al}, PNAS, 2005.}
